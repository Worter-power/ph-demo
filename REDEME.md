## 全局变量 
    ...
    13. 权限更新
        按钮或者页面权限控制说明
        1. 页面通过指定路由json文件具体路由上面code字段来动态过滤，如果没有指定code或者code值为false则该路由会默认加载
        2. 按钮权限
             2.1 通过vuex中codeKeys判断 $store.state.codeKeys[code] 结合v-if或者v-show实现，或者通过绑定style控制display属性实现
             2.2 通过指令v-ape 来实现 写法 v-ape="arg" arg为字符串或者数组，传入code
        规则：路由控制code 根据栏目树结构设置的code进行判断，如果存在二级或者三级路由，如果没有单独的控制二三级路由的按钮或者权限，则统一和对应的跟路由code一致
1.1.x
    1. STORE  // vuex全局变量
    2. 
